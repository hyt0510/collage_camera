...
- **リアルタイム同期**: API経由で10秒ごとに自動ポーリングを行い、投稿された写真データをモニターに反映。
...
3. **QRコードによる特定の枠の解放（場所限定お題）**
   `config.json` において、特定の枠（スロット）にロックIDを指定することで、会場内（例: 体育館や中庭）のQRコードをスキャンしないと撮影できない制限を設けています。
   - `themes` (撮影後のお題)、`lockMessages` (ロック中の案内)、`unlockMessages` (解除時の通知) をJSONで完全に分離し、柔軟な運用が可能。
   - URLパラメータ (`?unlock=xxx` や デバッグ用 `?lock=all`) でシームレスに状態を制御し、履歴（IndexedDB/localStorage）で進行状況を保持します。

4. **運営画面からの設定即時反映（キャッシュ回避と安全設計）**
   運営がAdmin画面でコラージュのプリセット（枠の数やお題）を変更した際、参加者の画面に即座に反映させるため以下の工夫を行っています。
   - Next.js (App Router) の Route Handler で `export const dynamic = "force-dynamic"` を指定し、APIのキャッシュを強制的に無効化。
   - `visibilitychange` や `focus` イベントを利用し、ブラウザのタブがアクティブになった瞬間にサーバーから最新設定を再取得。
   - 参加者がすでに1枚でも写真を撮影している場合は、進行中のコラージュが不意に消えてしまわないよう上書きを自動でブロックする安全機能を搭載。
